...
The first Phase C source slice now adds ordinary non-worker Pi registration on control port `12889`, bridge incarnation replacement, idempotent lifecycle events, heartbeat expiry, durable prompt claim/ack, and `wh pi sessions|events|prompt`. The normal Pi extension uses an outbound long-poll and `sendUserMessage`, aborting its handle on reload/session replacement; it never opens another published port. This slice has local API/CLI coverage but is not yet deployed or live-accepted. Global router, direct interactive terminal attachment, Zellij, and PWA remain future milestones.

**Current Phase A/B checklist:** A1 (delegation smoke), A2 (release provider config), A4 (acknowledged timeout/`sync`), and the deployed B1–B3 path are live-verified: a delegated child created a Unix-socket job, the worker persisted its canonical log and durable report, and the parent observed the linked `kind=delegated` job through the origin-session filter. A3 and B4 have regression coverage and current worker/orchestrator projections converge for the live job report. The approved archdome `WH_FAKEROOT=1` canary restored direct Tailscale SSH, generic `wh_dispatch`, and `wh_get_job_logs`, closing the linked-log transport blocker. The remaining live acceptance check is a provider-backed delegated model turn that itself chooses the overridden `bash` tool, followed by observation of its linked job and non-empty log. Permanent `410 Gone` ingest responses now durably abandon stale event/job outboxes so pre-upgrade projections cannot retry forever.
