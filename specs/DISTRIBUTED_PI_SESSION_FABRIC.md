...
The same private service overrides the child builtin `bash`: it is reachable only through a mode-`0600` Unix-domain socket at `${WH_DIR}/harness/pi-job/socket` (never a TCP listener), launches a canonical `wh_<job-id>` tmux job, persists its manifest and daemon-owned exit status, retains the existing harness log layout, and sends revisioned durable reports linked by `origin_session_id`. Parent job inspection remains the existing job API/tool surface with an origin-session filter. The public Tailscale-published terminal relay has no job-creation route. Local regression coverage and a real `pi-worker` launcher smoke prove the override; live end-to-end validation still requires deployment of the current orchestrator/worker images and activation of a rebuilt Pi release. Interactive-session bridge, global router, and PWA remain future milestones.

**Current Phase A/B checklist:** A1 (delegation smoke) and A2 (release provider config) are live-verified. A3 (durable worker event ingest), A4 (timeout/`termination_unknown`), A5 (`sync`), and B1–B4 (worker bash override, linked jobs, existing job inspection, tests) are implemented and regression-tested in source, but remain pending the combined deployment smoke. The M5 source gate is met; do not mark its live acceptance gate complete until the child performs a real command, the parent sees its linked log, and the worker/orchestrator projections converge.
