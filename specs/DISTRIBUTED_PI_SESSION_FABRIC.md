...
**Implementation status (2026-07-24):** the first worker-relay transport spike is implemented: a worker binds a loopback FastAPI/WebSocket relay on `WH_PI_RELAY_PORT` (default `27888`), publishes only that TCP port through userspace Tailscale Serve, advertises relay capability in its heartbeat schema, and removes its specific Serve rule during graceful daemon shutdown. Its current WebSocket route proves direct Tailnet transport and returns a typed `terminal: not_ready` status; delegated Pi launch, bridge registration, PTY forwarding, and orchestrator session APIs remain future milestones.
