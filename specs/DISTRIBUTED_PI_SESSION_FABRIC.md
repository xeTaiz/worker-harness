...
The same private service overrides the child builtin `bash`: it launches a canonical `wh_<job-id>` tmux job, persists its manifest and daemon-owned exit status, retains the existing harness log layout, and sends revisioned durable reports linked by `origin_session_id`. Parent job inspection remains the existing job API/tool surface with an origin-session filter. The public Tailscale-published terminal relay has no job-creation route. Local regression coverage and a real `pi-worker` launcher smoke prove the override; live end-to-end validation still requires deployment of the current orchestrator/worker images and activation of a rebuilt Pi release. Interactive-session bridge, global router, and PWA remain future milestones.
